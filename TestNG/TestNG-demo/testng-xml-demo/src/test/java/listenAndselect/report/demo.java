...
public class demo implements IReporter {
    ...
    private void init() {
        ...
        htmlReporter.config().setDocumentTitle("DiaoTest");
        htmlReporter.config().setReportName("DiaoTest");
        ...
        if (tests.size() > 0) {
            ...
            for (ITestResult result : treeSet) {
                ...
                while (logIterator.hasNext()){
                    ...
                    if(false){
                        String time = "2019-08-20 11:16";
                        log.setTimestamp(getTime(Long.valueOf(time)));
                        log.setDetails(details.substring(time.length()));
                    }else{
